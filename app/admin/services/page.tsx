...
export default function ServicesPage() {
  ...
  const [form, setForm] = useState({ name: "", description: "", icon: "", featured: false });
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (response.ok) {
      ...
      setForm({ name: "", description: "", icon: "", featured: false });
      await loadItems();
    } else {
      setMessage("Unable to save the service.");
    }
  };
    ...
    <div className="space-y-6">
      <div className="rounded-[2rem] border border-white/10 bg-slate-900/80 p-8 shadow-lg">
        <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-2xl font-semibold text-white">Manage services</h2>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-[2rem] border border-white/10 bg-slate-900/80 p-8 shadow-lg space-y-4">
        <div className="grid gap-4 md:grid-cols-2">
          <input value={form.name} onChange={(event) => setForm({ ...form, name: event.target.value })} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white" placeholder="Service name" required />
          <input value={form.icon} onChange={(event) => setForm({ ...form, icon: event.target.value })} className="rounded-2xl border border-white/10 bg-white/5 px-4 py-3 text-white" placeholder="Icon name" />
        </div>
        ...
        <div className="flex items-center gap-3">
          <button className="rounded-2xl bg-cyan-500 px-4 py-3 text-sm font-semibold text-slate-950 transition hover:bg-cyan-400">Save service</button>
          {message ? <span className="text-sm text-cyan-300">{message}</span> : null}
        </div>
      </form>

      <div className="space-y-3">
        {items.length === 0 ? <div className="rounded-2xl border border-white/10 bg-white/5 p-6 text-slate-300">No services yet.</div> : null}
        {items.map((item) => (
